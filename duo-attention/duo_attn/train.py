...
def main(args):
    ...
    if args.dataset_format == "multiple_passkey":
    ...
    elif args.dataset_format == "mds":
        class Dataset(torch.utils.data.Dataset):
            def __init__(self, dataset):
                from datatools import load
                self.dataset = load(args.dataset_name)
            ...
            def __getitem__(self, idx):
                ...
                if "mask" in item:
                    mask = torch.tensor(item["mask"]).bool()
                    labels[~mask] = -100

                if len(input_ids) < 8: # for seq parallel
                    pad = 8 - len(input_ids)
                    input_ids = torch.cat([input_ids, torch.zeros(pad, dtype=torch.long) + 128001], dim=0)
                    labels = torch.cat([labels, torch.zeros(pad, dtype=torch.long) - 100], dim=0)
        ...
        train_dataset = Dataset(args.dataset_name)
    else:
        raise ValueError(f"Dataset format {args.dataset_format} not supported")
