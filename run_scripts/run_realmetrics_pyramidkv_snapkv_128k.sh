...
JOB_NAME="ABLATE_${TAG}_helmet-${TASK_NAME}-${METHOD}-${SPARSITY}-${MODEL_NAME}"
    ...
    for METHOD in snapkv pyramidkv; do 
        for WITH_PATCH in true false; do
            for COMPRESS_GROUP_KVS in true false; do
                ...
                TASK=$TASK MODEL=$MODEL SUFFIX=$SUFFIX EXTRA=$EXTRA METHOD=$METHOD SPARSITY=$SPARSITY TAG=$TAG WITH_PATCH=$WITH_PATCH submit_job
            done
        done
    done
done
