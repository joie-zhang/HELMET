...
OUT_DIR=outputs/${MODEL_NAME}${TAG}/PATCH64_outputs${SUFFIX}__${METHOD}
...
JOB_NAME="FRESH_PATCH64-POOL_helmet-${TASK_NAME}-${METHOD}-${SPARSITY}-${MODEL_NAME}"
...
for TASK in ${tasks[@]} ; do 
    ...
    for METHOD in snapkv pyramidkv; do # pyramidkv
        for SPARSITY in 20 30 40 50 60 70 80 90; do # , 10 100
            # Convert to fraction
            ...
            TASK=$TASK MODEL=$MODEL SUFFIX=$SUFFIX EXTRA=$EXTRA METHOD=$METHOD SPARSITY=$SPARSITY WITH_PATCH=$WITH_PATCH FRESH=$FRESH submit_job
        done
    done
done
